...
export default function PopoverCE({ siteId }) {
  ...
  useEffect(() => {
    const checkAssignedOrNot = async () => {
      try {
        const formData = {
          siteId: siteId,
        };
        const data = await fetch(
          "http://localhost:4000/api/site/checkWhetherAssigned",
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    ...
    const viewAvailManagers = async () => {
      try {
        ...
        const data = await fetch(
          "http://localhost:4000/api/site/getManagers",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleSubmitConfirmation = async () => {
    if (selectedPerson) {
      try {
        ...
        const response = await fetch("http://localhost:4000/api/site/assignSiteManager", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleSubmitUnassign = async () => {
      try {
        ...
        const response = await fetch("http://localhost:4000/api/site/unassignSiteManager", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
            ...
            <ul role="list" className="divide-y divide-gray-100">
                ...
                {siteManagers.map((person) => (
                    ...
                    <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                        <p className="text-sm leading-6 text-gray-900">Site Manager</p>
                        {/* {person.lastSeen ? (

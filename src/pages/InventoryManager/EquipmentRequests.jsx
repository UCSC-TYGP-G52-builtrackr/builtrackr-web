...
const EquipmentRequests = () => {
  ...
  const handleApprove = async (request) => {
    try {
      ...
      const checkQuantityResponse = await fetch(
        `http://localhost:4000/api/erequest/checkEquipmentQuantity/${request.request_id}`,
        {
          method: "GET",
        }
      );
      ...
      if (available) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            fetch(
              `http://localhost:4000/api/erequest/approveEquipmentRequest/${request.request_id}`,
              {
                method: "PUT",
              }
            )
              .then((response) => {
                ...
                updateStatusLocally(request.request_id, "Approved");
                axios
                  .post("http://localhost:4000/api/employee/employeeForRequestAprove", {
  ...
  const handleReject = (requestId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `http://localhost:4000/api/erequest/rejectEquipmentRequest/${requestId}`,
          {
            method: "PUT",
          }
        )
  ...
  const handleClose = (requestId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `http://localhost:4000/api/erequest/deleteEquipmentRequest/${requestId}`,
          {
            method: "DELETE",
          }
        )
        ...
        <div className="md:pb-5 md:m-10 md:px-5 rounded-3xl">
          ...
          <div style={{ marginTop: "20px", width: "100%" }}>
            ...
            <ReactPaginate
              ...
              breakLabel={
                <span className="text-gray-600 px-2 py-1 rounded-md hover:bg-gray-100">
                  ...
                </span>
              }
              ...
              pageClassName={
                "bg-white text-gray-600 px-3 py-1 rounded-md hover:bg-gray-100"
              }
